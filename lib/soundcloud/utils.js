...
const id3 = R.curry((track, buffer) => {
  const artworkUrl = track.artwork_url && track.artwork_url.replace('large', 't500x500');
  const maybeArtwork = artworkUrl ? porygon.buffer(artworkUrl) : Promise.resolve();

  return maybeArtwork.then((artwork) => {
    ...
    if (artwork) writer.setFrame('APIC', artwork);
